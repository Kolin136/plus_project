...
import com.example.spring_plus.file.service.FileService;
...
import com.example.spring_plus.post.dto.CreatePostDto.Request;
...
public class PostService {
  ...
  public CreatePostDto.Response createPost(Request requestDto, List<MultipartFile> files,
      Long userId) {
    ...
    //클라가 전송한 파일 없으면 null 리턴
    if(files.get(0).isEmpty()) fileUrlList = null;
    //클라가 전송한 파일 있으면 저장 수행후 파일 url리턴
    else fileUrlList = fileService.fileSave(files, post);

    return Response.of(post, fileUrlList);
  }
